...
"use strict";ApplicationConfiguration.registerModule("addresses");
"use strict";ApplicationConfiguration.registerModule("applications");
"use strict";ApplicationConfiguration.registerModule("articles");
"use strict";ApplicationConfiguration.registerModule("bgchecks");
...
!function(){"use strict";function ProfilePictureController($scope,$timeout,$window,Authentication,FileUploader){$scope.user=Authentication.user,$scope.imageURL=$scope.user.profileImageURL,$scope.uploader=new FileUploader({url:"api/users/picture"}),$scope.uploader.filters.push({name:"imageFilter",fn:function(item){var type="|"+item.type.slice(item.type.lastIndexOf("/")+1)+"|";return-1!=="|jpg|png|jpeg|bmp|gif|".indexOf(type)}}),$scope.uploader.onAfterAddingFile=function(fileItem){if($window.FileReader){var fileReader=new FileReader;fileReader.readAsDataURL(fileItem._file),fileReader.onload=function(fileReaderEvent){$timeout(function(){$scope.imageURL=fileReaderEvent.target.result},0)}}},$scope.uploader.onSuccessItem=function(fileItem,response,status,headers){$scope.successFunction(fileItem,response,status,headers)},$scope.successFunction=function(fileItem,response){$scope.success=!0,$scope.user=Authentication.user=response,$scope.cancelUpload()},$scope.uploader.onErrorItem=function(fileItem,response){$scope.cancelUpload(),$scope.error=response.message},$scope.uploadProfilePicture=function(){$scope.success=$scope.error=null,$scope.uploader.uploadAll()},$scope.cancelUpload=function(){$scope.uploader.clearQueue(),$scope.imageURL=$scope.user.profileImageURL}}ProfilePictureController.$inject=["$scope","$timeout","$window","Authentication","FileUploader"],angular.module("users").controller("ChangeProfilePictureController",ProfilePictureController)}();
!function(){"use strict";function EditProfileController($http,$location,$log,Users,Authentication){var vm=this;vm.profile||$log.warn("[EditProfileController] No profile passed to osEditProfile Directive"),vm.profile=vm.profile||Authentication.user,$log.info("[EditProfileController] Profile %o",vm.profile),vm.updateUserProfile=function(isValid){if(isValid){vm.success=vm.error=null;var user=new Users(vm.profile);user.$update(function(response){vm.success=!0,Authentication.user=response},function(response){vm.error=response.data.message})}else vm.submitted=!0},vm.cancel=function(){alert("Cancel not yet implemented")}}EditProfileController.$inject=["$http","$location","$log","Users","Authentication"],angular.module("users").controller("EditProfileController",EditProfileController)}();
...
!function(){"use strict";function SettingsController($scope,$http,$stateParams,$location,$log,Users,Authentication){this.user=Authentication.user,this.user||$location.path("/"),$stateParams.userId&&($stateParams.userId===this.user._id||this.user.isAdmin||($log.warn("[SettingsController] Non-Admin tried to access another user's settings"),$location.path("/")))}SettingsController.$inject=["$scope","$http","$stateParams","$location","$log","Users","Authentication"],angular.module("users").controller("SettingsController",SettingsController)}();