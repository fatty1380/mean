...
var BGChecksController = function($scope, $stateParams, $location, $http, Authentication, Bgchecks, Stubs) {
    $scope.authentication = Authentication;

    // Watch for changes on the selected applicant
    $scope.$watch('applicant', function(newValue, oldValue) {
        if (newValue === oldValue) {
            console.log('no change in applicant - why did this get fired?');
            return;
        }
        ...
        $http.get('/bgcheck/login')
            .success(function(response) {
                $http.get('/bgcheck/applicants')
                    .success(function(response) {
                        console.log('got applicants: %o', response.applicants);
    ...
    $scope.selectApplicant = function(id) {
        console.log('Applicant %d selected ... loading info', id);
        $http.get('/bgcheck/applicants/' + id)
            .success(function(response) {
                debugger;
                console.log('got applicant info: %o', response);
...
BGChecksController.$inject = ['$scope', '$stateParams', '$location', '$http', 'Authentication', 'Bgchecks', 'StubValues'];
